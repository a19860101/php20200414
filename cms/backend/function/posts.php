<?php
    ...
    function showAllPosts($order="DESC"){
        try {
            ...
            $sql = "SELECT posts.*,category.title AS c_title,category.slug FROM posts LEFT JOIN category ON posts.cate_id = category.id ORDER BY id {$order}";
    ...
    function storePost($title,$content,$cate_id){
        try{
            global $pdo;
            $sql = "INSERT INTO posts(title,content,cate_id,create_at,update_at,cover)VALUES(?,?,?,?,?,?)";
            $stmt = $pdo->prepare($sql);
            $create_at = date("Y-m-d H:i:s");
            $cover = 0;
            $stmt->execute([$title,$content,$cate_id,$create_at,$create_at,$cover]);
        }catch(PDOException $e){
            echo $e->getMessage();
        }
    }
